...
# Management Group ID entry for extended scope
[void][Reflection.Assembly]::LoadWithPartialName('Microsoft.VisualBasic')
$titleMG = 'Enter Management Group ID'
$msgMG   = 'Please enter the Management Group ID for the analysis:'
$managementGroup = [Microsoft.VisualBasic.Interaction]::InputBox($msgMG, $titleMG)
Write-Host "Management Group selected: $managementGroup" -ForegroundColor Green
...
$resultK = search-azgraph -query "advisorresources | where type == 'microsoft.advisor/recommendations' | where properties.category == 'Cost' | where properties.shortDescription.solution contains 'Right-size' | project properties.extendedProperties.roleName, properties.extendedProperties.currentSku , properties.extendedProperties.targetSku, properties.extendedProperties.MaxMemoryP95, properties.extendedProperties.MaxCpuP95, resourceGroup, properties.shortDescription.solution" -Scope "managementGroups/$managementGroup" -first 1000
$resultK | Export-Csv -Path c:\azurecost\$costname\H-rightsize.csv -NoTypeInformation
 
$resultL = search-azgraph -query "advisorresources | where type == 'microsoft.advisor/recommendations' | where properties.category == 'Cost' | where properties.shortDescription.solution contains 'virtual machine reserved' | project properties.extendedProperties.annualSavingsAmount, properties.extendedProperties.reservationType, properties.extendedProperties.vmSize, properties.resourceMetadata.resourceId, properties.shortDescription.solution" -Scope "managementGroups/$managementGroup" -first 1000
$resultL | Export-Csv -Path c:\azurecost\$costname\G-AzureRI.csv -NoTypeInformation
 
$resultM = search-azgraph -query "resources | where type =~ 'microsoft.compute/disks' | where properties.diskState =~ 'reserved' | project name, properties.diskState, properties.diskSizeGB, location, resourceGroup, sku.name, sku.tier" -Scope "managementGroups/$managementGroup" -first 1000
$resultM | Export-Csv -Path c:\azurecost\$costname\M-Reserveddisk.csv -NoTypeInformation
 
$resultM = search-azgraph -query "Resources | summarize count() by type, location, resourceGroup" -Scope "managementGroups/$managementGroup" -first 1000
