...
function GetResourceGroupTags
{
  ...
  return $isTag,$resourceGroupTags
}
...
function GetResourceTags 
{
  ...
  $tagsJson = $tags | ConvertTo-Json -Compress
  # Add to result object array
  $resourceTags += SetObjResult @($subscription.Name, $subscription.Id, $resourceGroupName, $resource.Name, $resource.ResourceType, $resource.ResourceId, $resource.Location, $tagsJson)
  return $isTag,$resourceTags
}
...
if ( (CreateDirectoryResult $globalVar.pathResult) ) {
  ...
  $csvFileTags = (CreateFile -pathName $globalVar.pathResult -fileName $globalVar.FileWithTags -extension 'csv' -chrono $globalVar.chronoFile)
  # Create csv file with No Tag
  $csvFileNoTags = (CreateFile -pathName $globalVar.pathResult -fileName $globalVar.fileWithNoTag -extension 'csv' -chrono $globalVar.chronoFile)
  # Create the xlsx file results
  $xlsResFile = (CreateFile -pathName $globalVar.pathResult -fileName $globalVar.fileResultXls -extension 'xlsx' -chrono $globalVar.chronoFile)
  # if generateLogFile in Json file is set to "Y", create log file
  if ($globalVar.generateLogFile.ToUpper() -eq "Y") {
    # Create log file
    $globalLog = $true
    $logfile = (CreateFile -pathName $globalVar.pathResult -fileName $globalVar.fileResultXls -extension 'log' -chrono $globalVar.chronoFile)
  }
}
...
if ($subscriptions.Count -ne 0) {
  foreach ($subscription in $subscriptions) {
    ...
    $isTag,$arrayTags = (GetSubscriptionTags -subscription $subscription)
    if ($isTag) {
      $arrayTags | Export-Csv -Path $csvFileTags -Delimiter ";" -NoTypeInformation -Append
    }
    else {
      $arrayTags | Export-Csv -Path $csvFileNoTags -Delimiter ";" -NoTypeInformation -Append
    }
    ...
    if ($resourceGroupsCount -ne 0) {
      foreach ($resourceGroup in $resourceGroups) {
        ...
        $isTag,$arrayTags = (GetResourceGroupTags -subscription $subscription -ResourceGroup $resourceGroup)
        if ($isTag) {
          $arrayTags | Export-Csv -Path $csvFileTags -Delimiter ";" -NoTypeInformation -Append
        }
        else {
        ...
        if ($numberOfResources -ne 0) {
          ...
          foreach ($resource in $resources) {
            $isTag,$arrayTmpTags = (GetResourceTags -subscription $subscription -resourceGroupName $resourceGroup.ResourceGroupName -resource $resource)
            if ($isTag) { $arrayTags += $arrayTmpTags }
            else { $arrayNoTags += $arrayTmpTags }
            $countResource += 1
            ...
            if ($countResource -eq $globalVar.saveEvery) {
              if ($arrayTags.Count -gt 0) {$arrayTags | Export-Csv -Path $csvFileTags -Delimiter ";" -NoTypeInformation -Append }
              if ($arrayNoTags.Count -gt 0) {$arrayNoTags | Export-Csv -Path $csvFileNoTags -Delimiter ";" -NoTypeInformation -Append }
              $arrayTags = @()
              $arrayNoTags = @()
              $countResource = 0
            }
