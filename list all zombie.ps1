...
New-Item -Path "c:\" -Name "Azurecost\zombie\$costname" -ItemType "directory" -force
set-location c:\azurecost\zombie\$costname

# FinOps queries using Azure graph
$resultA = Search-AzGraph -query "resources | where type =~ 'microsoft.compute/disks' | where properties.diskState =~ 'Unattached' | extend optimization = 'Disks_unattached' | project optimization, name, id, subscriptionId, resourceGroup, location, owner_contact=tags.owner_contact, operational_contact=tags.operational_contact, owner_dl=tags.owner_dl, financial_contact=tags.financial_contact, app_id=tags.app_id, billing_code=tags.billing_code, app_family=tags.app_family"  -first 1000 
$resultA | Export-Csv -Path c:\Azurecost\zombie\$costname\A-disk.csv -NoTypeInformation 

$resultB = Search-AzGraph -query "resources | where type =~ 'microsoft.web/serverfarms' | where properties.numberOfSites == 0 | extend Details = pack_all() | extend optimization = 'App_service' | project optimization, name, id, subscriptionId, resourceGroup, location, owner_contact=tags.owner_contact, operational_contact=tags.operational_contact, owner_dl=tags.owner_dl, financial_contact=tags.financial_contact, app_id=tags.app_id, billing_code=tags.billing_code, app_family=tags.app_family" -first 1000 
$resultB | Export-Csv -Path c:\Azurecost\zombie\$costname\B-AppServ.csv -NoTypeInformation 

$resultC = Search-AzGraph -query "Resources | where type == 'microsoft.network/publicipaddresses' | extend optimization = 'Public_IP' | where properties.ipConfiguration == '' and properties.natGateway == '' and properties.publicIPPrefix == '' | extend Details = pack_all() | project optimization, name, id, subscriptionId, resourceGroup, location, owner_contact=tags.owner_contact, operational_contact=tags.operational_contact, owner_dl=tags.owner_dl, financial_contact=tags.financial_contact, app_id=tags.app_id, billing_code=tags.billing_code, app_family=tags.app_family" -first 1000
$resultC | Export-Csv -Path c:\Azurecost\zombie\$costname\C-IP.csv -NoTypeInformation 

$resultD = Search-AzGraph -query "Resources | where type has 'microsoft.network/networkinterfaces' | where isnull(properties.privateEndpoint) | extend optimization = 'NIC'| where isnull(properties.privateLinkService) | where properties.hostedWorkloads == '[]' | where properties !has 'virtualmachine' | extend Details = pack_all() | extend AllProperties = todynamic(tags) | project optimization, name, id, subscriptionId, resourceGroup, location, owner_contact=tags.owner_contact, operational_contact=tags.operational_contact, owner_dl=tags.owner_dl, financial_contact=tags.financial_contact, app_id=tags.app_id, billing_code=tags.billing_code, app_family=tags.app_family" -First 1000
$resultD | Export-Csv -Path c:\Azurecost\zombie\$costname\D-NIC.csv -NoTypeInformation 

$resultE = Search-AzGraph -query "ResourceContainers | where type == 'microsoft.resources/subscriptions/resourcegroups'  | extend optimization = 'RG_empty' |extend rgAndSub = strcat(resourceGroup, '--', subscriptionId)  | join kind=leftouter (Resources  | extend rgAndSub = strcat(resourceGroup, '--', subscriptionId)      | summarize count() by rgAndSub  ) on rgAndSub  | where isnull(count_)  | extend Details = pack_all()  | project optimization, name, id, subscriptionId, resourceGroup, location, owner_contact=tags.owner_contact, operational_contact=tags.operational_contact, owner_dl=tags.owner_dl, financial_contact=tags.financial_contact, app_id=tags.app_id, billing_code=tags.billing_code, app_family=tags.app_family" -first 1000
$resultE | Export-Csv -Path c:\Azurecost\zombie\$costname\E-RG.csv -NoTypeInformation 

$resultF = Search-AzGraph -query "Resources | where type == 'microsoft.network/networksecuritygroups' and isnull(properties.networkInterfaces) and isnull(properties.subnets) | extend optimization = 'NSG' |extend Details = pack_all() | project optimization, name, id, subscriptionId, resourceGroup, location, owner_contact=tags.owner_contact, operational_contact=tags.operational_contact, owner_dl=tags.owner_dl, financial_contact=tags.financial_contact, app_id=tags.app_id, billing_code=tags.billing_code, app_family=tags.app_family" -first 1000
$resultF | Export-Csv -Path c:\Azurecost\zombie\$costname\F-NSG.csv -NoTypeInformation 


$resultG = Search-AzGraph -query "resources | where type == 'microsoft.network/routetables' | extend optimization = 'Route_tables' |where isnull(properties.subnets) | extend Details = pack_all() | project optimization, name, id, subscriptionId, resourceGroup, location, owner_contact=tags.owner_contact, operational_contact=tags.operational_contact, owner_dl=tags.owner_dl, financial_contact=tags.financial_contact, app_id=tags.app_id, billing_code=tags.billing_code, app_family=tags.app_family" -first 1000
$resultG | Export-Csv -Path c:\Azurecost\zombie\$costname\G-RT.csv -NoTypeInformation 

$resultH = Search-AzGraph -query "advisorresources  | where type == 'microsoft.advisor/recommendations' | where properties.category == 'Cost' | extend optimization = 'VM' | where properties.shortDescription.solution contains 'Right-size'  | where properties.extendedProperties.recommendationType =='Shutdown' | project optimization, name=properties.extendedProperties.roleName, id, subscriptionId, resourceGroup, location=properties.extendedProperties.regionId,'','','','','','','','' " -first 1000
...
#Write-Host "Concaténation des fichiers CSV terminée. Les données ont été enregistrées dans $cheminichierSortie."
...
$contenuFinal | Set-Content $cheminFichierCSV
